...
function PolicyBadge({ policy }: { policy: string }) {
  const styles = {
    HIGHEST_PRIORITY_WINS: { class: 'badge-primary', label: 'Highest Priority Wins' },
...
export default function CampaignGroupDetailPage() {
  ...
  const [collisionPolicy, setCollisionPolicy] = useState<CampaignGroup['collisionPolicy']>('HIGHEST_PRIORITY_WINS');
  ...
  const fetchGroup = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch campaign group');
    } finally {
      setIsLoading(false);
    }
  }, [id]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to update campaign group');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete this campaign group? This action cannot be undone.')) {
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete campaign group');
      setIsDeleting(false);
    }
  };
      ...
      <div className="card">
        <div className="empty-state">
          ...
          <p className="empty-state-description">
            The campaign group you&apos;re looking for doesn&apos;t exist or has been deleted.
          </p>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setCollisionPolicy(e.target.value as CampaignGroup['collisionPolicy'])}
                  disabled={isSaving}
                >
                  <option value="HIGHEST_PRIORITY_WINS">Highest Priority Wins</option>
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-muted)] mt-2">
                  Time period to check for previous sends when detecting collisions.
                </p>
              </div>
            </div>
          ...
          <div className="card-glow">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-[var(--text-secondary)]">Campaigns</span>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-secondary)]">
                  Campaigns in this group share collision detection. When the same user
                  qualifies for multiple campaigns, the policy determines which email sends.
                </p>
              </div>
            </div>
          </div>
          ...
          {group._count?.singleSends && group._count.singleSends > 0 && (
            <div className="card-glow">
              ...
              >
                ...
                View {group._count.singleSends} Campaign{group._count.singleSends !== 1 ? 's' : ''}
              </Link>
            </div>
          )}
        </div>
