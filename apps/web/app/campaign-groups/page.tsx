    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-[var(--bg-secondary)] border border-[var(--border-default)] rounded-2xl w-full max-w-lg p-6 shadow-2xl animate-slide-up">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-[var(--text-primary)]">Create Campaign Group</h2>
          <button onClick={handleClose} className="btn btn-ghost p-2" disabled={isSubmitting}>
            <X className="w-5 h-5" />
          </button>
        </div>
...
export default function CampaignGroupsPage() {
  ...
  const fetchGroups = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch campaign groups');
    } finally {
      setIsLoading(false);
    }
  }, []);
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[var(--text-secondary)]">
            Group related campaigns to prevent email fatigue with collision detection
          </p>
        </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              When multiple campaigns in the same group target the same user, the collision policy
              determines which email gets sent. With "Highest Priority Wins", only the highest
              priority campaign sends within the collision window.
            </p>
          </div>
        </div>
      </div>
              ...
              <div key={group.id} className="card-glow group">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-[var(--border-default)]">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Link href={`/campaign-groups/${group.id}/edit`} className="btn btn-ghost p-2">
        ...
        <div className="card">
          <div className="empty-state">
            ...
            <p className="empty-state-description">
              Create your first campaign group to start organizing campaigns and prevent email fatigue
            </p>
            <button onClick={() => setIsModalOpen(true)} className="btn btn-primary">
              <Plus className="w-4 h-4" />
              Create Group
