...
import type { Campaign, Template, Segment, SenderProfile, CampaignRun } from '@/lib/api';
...
export default function CampaignEditor({
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      setSuccessMessage('Campaign triggered successfully');
      setTimeout(() => setSuccessMessage(null), 3000);
      await fetchData();
    } catch (err) {
            ...
            {campaign.runs && campaign.runs.length > 0 ? (
              <div className="space-y-3">
                {campaign.runs.map((run) => (
                  <div
                    key={run.id}
                    className="flex items-center justify-between p-3 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-default)]"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {run.stats && (
                          <p className="text-xs text-[var(--text-muted)]">
                            Sent: {run.stats.sent} | Failed: {run.stats.failed} | Skipped: {run.stats.skipped}
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="text-center py-8 text-[var(--text-muted)]">
                No runs yet. Click <span className="text-[var(--text-primary)] font-medium">Trigger</span> to start.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card-glow">
            ...
            <div className="space-y-3">
              ...
              <div>
                <span className="text-xs text-[var(--text-muted)]">Segment</span>
                <p className="text-sm text-[var(--text-primary)]">
                  {campaign.segment?.name || 'Unknown'}
                </p>
              </div>
              <div>
                <span className="text-xs text-[var(--text-muted)]">Sender</span>
                <p className="text-sm text-[var(--text-primary)]">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-secondary)]">
                  {campaign.scheduleType === 'CRON' ? (
                    <>
                      Scheduled: <code className="text-indigo-400">{campaign.cronExpression}</code>
                    </>
                  ) : (
                    'Manual trigger only'
                  )}
          ...
          <div className="card-glow">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-secondary)]">
                  Lower priority number means higher importance. Used for collision
                  detection when multiple campaigns target the same user.
                </p>
              </div>
            </div>
          </div>
