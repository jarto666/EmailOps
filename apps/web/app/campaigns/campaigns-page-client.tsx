...
export default function CampaignsPageClient({
  ...
  const [cronExpression, setCronExpression] = useState("0 * * * *"); // hourly default
  ...
  async function load() {
    ...
    setError(null);
    try {
      const [campaignsData, templatesData, segmentsData, senderProfilesData] =
        await Promise.all([
          apiFetch<Campaign[]>("/campaigns", { query: { workspaceId } }),
      ...
      <div className="bg-white border rounded p-4">
        ...
        {scheduleType === "CRON" ? (
          <label className="text-sm block mt-3">
            ...
            <div className="text-xs text-gray-500 mt-1">
              5 or 6 fields. Example hourly: <span className="font-mono">0 * * * *</span>
            </div>
          </label>
        ) : null}
        ...
        <label className="text-sm block mt-3">
          ...
          <input
            ...
            onChange={(e) => setRateLimitPerSecond(parseInt(e.target.value || "10", 10))}
          />
        </label>
            ...
            <div key={c.id} className="px-4 py-3 flex items-center justify-between">
              <div>
                <div className="font-medium">
                  <Link
                    className="text-indigo-700 hover:underline"
                    href={`/campaigns/${c.id}?workspaceId=${encodeURIComponent(
                      workspaceId
                    )}`}
                  >
                    {c.name}
                ...
                <div className="text-xs text-gray-600">
                  {c.status} · {c.scheduleType}
                  {c.scheduleType === "CRON" && c.cronExpression
                    ? ` · ${c.cronExpression}`
                    : ""}
                </div>
                <div className="text-xs text-gray-500 mt-0.5">
                  Template: <span className="font-mono">{c.template?.name ?? "—"}</span>{" "}
                  · Segment: <span className="font-mono">{c.segment?.name ?? "—"}</span>{" "}
                  · From:{" "}
                  <span className="font-mono">
                    {c.senderProfile?.fromEmail ?? "—"}
                  </span>
                </div>
              </div>
